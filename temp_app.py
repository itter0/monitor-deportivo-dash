...
try:
    ...
    def get_coach_dashboard(username, full_name, current_search="", COLORS=None, STYLES=None):
        return html.Div("Error: Coach Dashboard module not found", style={'color': 'red'})

try:
    from nutrition_module import get_nutritionist_dashboard, calculate_macros, predict_weight_cut, create_weight_cut_graph
except ImportError:
    def get_nutritionist_dashboard(username, full_name, current_search="", COLORS=None, STYLES=None):
        return html.Div("Error: Nutrition module not found", style={'color': 'red'})
    def calculate_macros(*args, **kwargs):
        return {}
    def predict_weight_cut(*args, **kwargs):
        return {}
    def create_weight_cut_graph(*args, **kwargs):
        return html.Div("Nutrition graph unavailable", style={'color': 'red'})
# --------------------------------------------------------------------------
...
COLORS = {
    ...
    'muted': '#CCCCCC',        # ✨ MEJORADO: De #888888 a #CCCCCC (mejor legibilidad en móvil)
    'text_muted': '#CCCCCC',   # ✨ Consistente con muted
    'border_neon': '#333333',  # ✨ CAMBIO: De #ff0000 a #333333 (gris para bordes normales)
    'border_accent': '#ff0000', # ✨ NUEVO: Reservar rojo SOLO para alertas críticas
...
STYLES = {
    'main_container': {
        ...
        'color': COLORS['text'],
        'padding': '20px',
        'fontSize': '16px'  # ✨ NUEVO: Base para escalado responsive
    },
    'login_container': {
        'maxWidth': '450px',
        'margin': '80px auto',
        ...
        'borderRadius': '10px',
        'border': f'2px solid {COLORS["border_neon"]}',  # Ahora es gris, no rojo
        'boxShadow': '0 0 20px rgba(51, 51, 51, 0.4)',   # ✨ CAMBIO: Resplandor gris
        'color': COLORS['text'],
        'textAlign': 'center',
    ...
    'navbar': {
        'background': '#000000',
        'padding': '15px 40px',
        'borderBottom': f'2px solid {COLORS["border_soft"]}',  # ✨ CAMBIO: Gris suave
        'display': 'flex',
    ...
    'card': {
        ...
        'boxShadow': '0 0 15px rgba(51, 51, 51, 0.3)',   # ✨ CAMBIO: Resplandor gris
        'marginBottom': '20px',
        'position': 'relative',
        'overflow': 'hidden',
        'fontFamily': "'Oswald', 'Segoe UI', 'Roboto', sans-serif"  # ✨ NUEVO
    ...
    'card_header_tactical': {
        ...
        'fontSize': '1.4rem',  # ✨ CAMBIO: Más legible
        'marginBottom': '15px',
        'letterSpacing': '2px',  # ✨ AUMENTADO: Más espacio
    ...
    'button_primary': {
        ...
        'border': f'2px solid {COLORS["border_soft"]}',  # ✨ CAMBIO: Gris por defecto
        'color': 'white',
        'textTransform': 'uppercase',
            ...
            html.Div([
                ...
                dcc.Dropdown(
                    ...
                    options=[
                        ...
                        {'label': '🏋️ Entrenador (Head Coach)', 'value': 'entrenador'},
                        {'label': '🥗 Nutricionista', 'value': 'nutricionista'}  # ✨ NUEVO
                    ],
...
def get_view_appointments_layout_patient(username, full_name, current_search=""):
    ...
    def build_appointment_card(app, category):
        ...
        if category == 'pending':
            actions = html.Div([
                dbc.Button("✅ Confirmar", id={'type': 'confirm-appt-patient-btn', 'index': app['id']}, 
                          color="success", size="sm", className="w-100"),
                dbc.Button("❌ Cancelar", id={'type': 'cancel-appt-patient-btn', 'index': app['id']}, 
                          color="danger", size="sm", className="w-100"),
            ], className="d-grid gap-2")
        elif category == 'upcoming':
            actions = html.Div([
                dbc.Button("❌ Cancelar Cita", id={'type': 'cancel-appt-patient-btn', 'index': app['id']}, 
        ...
        if category == 'past':
            ...
            notes_display = html.Div([
                html.P([html.Strong("Notas del Médico: ", style={'color': '#ffffff'}), 
                       html.Span(doctor_notes, style={'color': COLORS['muted']})], 
                      style={'color': '#ffffff', 'marginBottom': '8px', 'fontSize': '0.95rem'}),
                html.P([html.Strong("Estado: ", style={'color': '#ffffff'}), 
                       html.Span(status_text, className=f"text-{status_color}")], 
                      style={'color': '#ffffff', 'fontSize': '0.95rem'})
            ], style={'marginTop': '15px', 'padding': '12px', 'border': f'1px solid {COLORS["border_soft"]}', 
                     'borderRadius': '5px', 'backgroundColor': '#111111'})

        return dbc.Card(
            dbc.CardBody([
                html.H5(f"👨‍⚕️ {app['professional_name']}", 
                       className="card-title", 
                       style={'color': COLORS['primary'], 'marginBottom': '12px', 'fontSize': '1.2rem'}),
                
                # ✨ CAMBIO: Información más compacta en móvil
                html.Div([
                    html.P(f"📅 {appt_dt.strftime('%d/%m/%Y %H:%M')}", 
                          className="card-text mb-2", 
                          style={'color': '#ffffff', 'fontSize': '0.95rem'}),
                    html.P(f"🏥 {app['hospital']} - {app['office']}", 
                          className="card-text mb-2", 
                          style={'color': '#ffffff', 'fontSize': '0.95rem'}),
                    html.P(f"📝 {app['comments']}", 
                          className="card-text mb-2", 
                          style={'color': COLORS['muted'], 'fontSize': '0.9rem', 'lineHeight': '1.4'}),
                ]),
            ...
            className="mb-3",
            style={'backgroundColor': '#0a0a0a', 'border': f'2px solid {COLORS["border_neon"]}', 
                  'color': '#ffffff', 'borderRadius': '8px', 'padding': '16px'}
        )
    ...
    return html.Div([
        ...
        html.Div([
            ...
            dbc.Button("← Volver", id="nav-dashboard-btn-patient-appt", href=f"/{current_search}", 
                      color="secondary", size="sm", className="mb-3 w-100"),
            
            # --- Citas Pendientes ---
            html.Div([
                html.H4("🚨 Pendientes de Confirmación", 
                       style={'color': COLORS['primary'], 'marginBottom': '15px', 'fontSize': '1.3rem'}),
                html.Div(
                    [build_appointment_card(app, 'pending') for app in pending_apps]
                ) if pending_apps else html.P("✅ No tienes citas pendientes.", 
                                             style={'padding': '20px', 'backgroundColor': '#1a1b1e', 
                                                   'borderRadius': '5px', 'color': COLORS['muted']})
            ], style={**STYLES['card'], 'marginBottom': '20px'}),

            # --- Próximas Citas ---
            html.Div([
                html.H4("✅ Próximas Citas", 
                       style={'color': COLORS['primary'], 'marginBottom': '15px', 'fontSize': '1.3rem'}),
                html.Div(
                    [build_appointment_card(app, 'upcoming') for app in upcoming_apps]
                ) if upcoming_apps else html.P("📅 No hay citas confirmadas próximas.", 
                                              style={'padding': '20px', 'backgroundColor': '#1a1b1e', 
                                                    'borderRadius': '5px', 'color': COLORS['muted']})
            ], style={**STYLES['card'], 'marginBottom': '20px'}),
            ...
            html.Div([
                ...
                ) if past_apps else html.P("📭 No hay historial de citas.", 
                                          style={'padding': '20px', 'backgroundColor': '#1a1b1e', 
                                                'borderRadius': '5px', 'color': COLORS['muted']})
            ], style=STYLES['card']),
            
            html.Div(id='patient-appt-action-feedback', className="mt-3")
            
        ], className="px-2 px-md-4", style={'paddingBottom': '20px'}),  # ✨ NUEVO: Padding responsive
    ], style=STYLES['main_container'])
    ...
    if user_session and user_session.get('username'):
        ...
        if pathname in ['/', '/login', '/register']:
            if role == 'medico':
                ...
                return get_coach_dashboard(username, full_name, session_search, COLORS, STYLES), updated_session, dash.no_update 
            elif role == 'nutricionista':  # ✨ NUEVO
                return get_nutritionist_dashboard(username, full_name, session_search, COLORS, STYLES), updated_session, dash.no_update
...
def calculate_macros_callback(weight, height, age, gender, activity, goal):
    if not all([weight, height, age, gender, activity, goal]):
        return html.Div("Complete todos los campos", style={"color": "#ff6b6b"})
    
    try:
        macros = calculate_macros(weight, height, age, gender, activity, goal)
        
        goal_text = {"maintain": "Mantener", "cut": "Corte", "bulk": "Ganar Masa"}[goal]
        
        return html.Div([
            html.H5(f"Plan Nutricional - {goal_text}", style={"color": "#00ff00", "marginBottom": "15px"}),
            dbc.Row([
                dbc.Col([
                    html.Div([
                        html.H4(f"{macros['calories']}", style={"color": "#ff0000", "fontSize": "2rem", "fontWeight": "bold"}),
                        html.P("Calorías/día", style={"color": "#CCCCCC", "margin": "0"})
                    ], style={"textAlign": "center", "padding": "15px", "background": "#1a1a1a", "borderRadius": "8px"})
                ], md=3),
                dbc.Col([
                    html.Div([
                        html.H4(f"{macros['protein']}g", style={"color": "#00ff00", "fontSize": "2rem", "fontWeight": "bold"}),
                        html.P("Proteínas", style={"color": "#CCCCCC", "margin": "0"})
                    ], style={"textAlign": "center", "padding": "15px", "background": "#1a1a1a", "borderRadius": "8px"})
                ], md=3),
                dbc.Col([
                    html.Div([
                        html.H4(f"{macros['carbs']}g", style={"color": "#ff6b35", "fontSize": "2rem", "fontWeight": "bold"}),
                        html.P("Carbohidratos", style={"color": "#CCCCCC", "margin": "0"})
                    ], style={"textAlign": "center", "padding": "15px", "background": "#1a1a1a", "borderRadius": "8px"})
                ], md=3),
                dbc.Col([
                    html.Div([
                        html.H4(f"{macros['fats']}g", style={"color": "#ffff00", "fontSize": "2rem", "fontWeight": "bold"}),
                        html.P("Grasas", style={"color": "#CCCCCC", "margin": "0"})
                    ], style={"textAlign": "center", "padding": "15px", "background": "#1a1a1a", "borderRadius": "8px"})
                ], md=3)
            ])
        ])
    except Exception as e:
        return html.Div(f"Error en cálculo: {str(e)}", style={"color": "#ff6b6b"})

@app.callback(
    [Output("weight-cut-graph", "figure"), Output("cut-prediction-info", "children")],
...
def update_weight_cut_graph(current_weight, target_weight, weeks):
    if not all([current_weight, target_weight, weeks]):
        return go.Figure(), html.Div("Complete todos los campos", style={"color": "#ff6b6b"})
    
    try:
        # Estimar calorías basadas en macros (asumiendo corte agresivo)
        estimated_calories = current_weight * 25  # ~25 cal/kg para corte
        
        prediction = predict_weight_cut(current_weight, target_weight, weeks, estimated_calories)
        
        if prediction.get("warning"):
            return go.Figure(), html.Div(prediction["warning"], style={"color": "#ff6b6b"})
        
        fig = create_weight_cut_graph(prediction, current_weight, target_weight)
        
        info = html.Div([
            html.P(f"📊 Pérdida estimada: {prediction.get('total_loss', 0)} kg en {weeks} semanas", 
                   style={"color": "#00ff00", "fontWeight": "bold", "marginBottom": "8px"}),
            html.P(f"⚡ Ritmo: {prediction.get('kg_per_week', 0)} kg por semana", 
                   style={"color": "#ffff00", "marginBottom": "8px"}),
            html.P(f"🎯 Peso final proyectado: {prediction['weights'][-1]:.1f} kg", 
                   style={"color": "#CCCCCC"})
        ])
        
        return fig, info
    except Exception as e:
        return go.Figure(), html.Div(f"Error en predicción: {str(e)}", style={"color": "#ff6b6b"})
